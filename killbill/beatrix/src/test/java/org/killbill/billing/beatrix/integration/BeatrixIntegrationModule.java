...
public class BeatrixIntegrationModule extends KillBillModule {
    ...
    public static final String BUILT_IN_PLUGIN_NAME = "yoyo";

        public static final String EXTERNAL_PLUGIN_NAME = "external-payment-plugin";
    ...
    void bindKillbillActivatorDependencies() {
        // KillbillServerModule has one PluginServiceRegistry<Healthcheck> binding. But I suspect that module not used
        // in beatrix, so we need one.
        OptionalBinder.newOptionalBinder(binder(), new TypeLiteral<PluginServiceRegistry<Healthcheck>>() {}).setDefault().toProvider(Providers.of(null));

        // killbill-platform-test have this one (and most of other deps), but it's test classes.
        OptionalBinder.newOptionalBinder(binder(), new TypeLiteral<PluginServiceRegistry<ServiceDiscoveryRegistry>>() {}).setDefault().toProvider(Providers.of(null));
        OptionalBinder.newOptionalBinder(binder(), new TypeLiteral<SinglePluginServiceRegistry<MetricRegistry>>() {}).setDefault().toProvider(Providers.of(null));
    ...
    private static final class PaymentPluginMockModule extends PaymentModule {
        ...
        protected void installPaymentProviderPlugins(final PaymentConfig config) {
            install(new MockPaymentProviderPluginModule(BUILT_IN_PLUGIN_NAME, clock, configSource));
        }
    }
