...
public class TestEntitlement extends TestJaxrsBase {
    ...
    public void testCreateSubscriptionWithOnlyUsageOverrides() throws Exception {
        ...
        final Subscription subscription = subscriptionApi.getSubscription(entitlementJson.getSubscriptionId(), requestOptions);
        Assert.assertEquals(subscription.getState(), EntitlementState.ACTIVE);
        // The overridden plan is named after its parent plus the database record id of the override
        // definition (see DefaultPriceOverrideSvc). Asserting the shape rather than "-1" keeps the
        // check meaningful without requiring a virgin sequence: MySQL restarts AUTO_INCREMENT with
        // each fresh embedded database, but a reused PostgreSQL does not, so the number depends on
        // how many overrides earlier tests created.
        final String planName = subscription.getPlanName();
        Assert.assertTrue(planName.matches("trebuchet-usage-in-arrear-\\d+"),
                          "Unexpected overridden plan name: " + planName);
