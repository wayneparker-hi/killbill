...
public class PluginResource extends JaxRsResourceBase {
    ...
    private final HttpServlet pluginServlet;

    @Inject
    public PluginResource(@Named(PluginServiceProperties.PLUGIN_SERVLET) final HttpServlet pluginServlet,
        ...
        this.pluginServlet = pluginServlet;
    }
    ...
    public Response doDELETE(@jakarta.ws.rs.core.Context final HttpServletRequest request,
        ...
        return serviceViaPlugin(request, response, servletContext, servletConfig, uriInfo);
    }
    ...
    public Response doFormPOST(final MultivaluedMap<String, String> form,
                               ...
                               @jakarta.ws.rs.core.Context final ServletConfig servletConfig,
                               @jakarta.ws.rs.core.Context final UriInfo uriInfo) throws ServletException, IOException {
        return serviceViaPlugin(form, request, response, servletContext, servletConfig, uriInfo);
    }
    ...
    public Response doHEAD(@jakarta.ws.rs.core.Context final HttpServletRequest request,
                           ...
                           @jakarta.ws.rs.core.Context final ServletConfig servletConfig,
                           @jakarta.ws.rs.core.Context final UriInfo uriInfo) throws ServletException, IOException {
        serviceViaPlugin(request, response, servletContext, servletConfig, uriInfo);

        // Make sure to return 204
        return Response.noContent().build();
    }

    private Response serviceViaPlugin(final HttpServletRequest request, final HttpServletResponse response,
        ...
        return serviceViaPlugin(request, request.getInputStream(), null, response, servletContext, servletConfig, uriInfo);
    }

    private Response serviceViaPlugin(final MultivaluedMap<String, String> form,
        ...
        return serviceViaPlugin(request, createInputStream(request, form), form, response, servletContext, servletConfig, uriInfo);
    }

    private Response serviceViaPlugin(final HttpServletRequest request, final InputStream inputStream, @Nullable final MultivaluedMap<String, String> formData,
        ...
        preparePluginRequest(request, servletContext, servletConfig);

        final ServletRequest req = new PluginServletRequestWrapper(request, inputStream, formData, uriInfo.getQueryParameters());
        ...
        final PluginServletResponseWrapper res = new PluginServletResponseWrapper(response);
    ...
    private void preparePluginRequest(final HttpServletRequest request, final ServletContext servletContext, final ServletConfig servletConfig) {
        request.setAttribute("killbill.plugin.servletContext", servletContext);
        request.setAttribute("killbill.plugin.servletConfig", servletConfig);
    }

    // Request wrapper: hides the /plugins prefix from the plugin's servlet and buffers the body
    private static final class PluginServletRequestWrapper extends HttpServletRequestWrapper {

        private final InputStream inputStream;
        private final Map<String, String[]> parameterMap;

        public PluginServletRequestWrapper(final HttpServletRequest request, final InputStream inputStream, @Nullable final MultivaluedMap<String, String> formData, final MultivaluedMap<String, String> queryParameters) {
            super(request);
    ...
    private static final class PluginServletResponseWrapper extends HttpServletResponseWrapper {
        ...
        // captured bytes after the plugin servlet returns and hands them to Jersey.
        // See https://github.com/killbill/killbill/issues/2205
        ...
        public PluginServletResponseWrapper(final HttpServletResponse response) {
            super(response);
        }
        ...
        /**
         * @return the bytes captured during the plugin servlet invocation; flushes the lazily
