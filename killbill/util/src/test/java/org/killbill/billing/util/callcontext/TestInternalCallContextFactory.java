    ...
    public void testCreateInternalCallContextWithAccountRecordIdFromSimpleObjectType() throws Exception {
        ...
        dbi.withHandle(new HandleCallback<Void>() {
            ...
            public Void withHandle(final Handle handle) throws Exception {
                // grp_id is declared NOT NULL with no default, so it has to be supplied explicitly.
                // Omitting it worked only on MySQL, whose non-strict mode silently substitutes an
                // empty string; PostgreSQL rejects the row.
                handle.execute("insert into invoices (id, account_id, invoice_date, target_date, currency, status, migrated, created_by, created_date, account_record_id, tenant_record_id, grp_id) values " +
                               "(?, ?, ?, ?, 'USD', 'COMMITTED', FALSE, 'test', ?, ?, ?, ?)", invoiceId.toString(), UUID.randomUUID().toString(), new Date(), new Date(), new Date(), accountRecordId, internalCallContext.getTenantRecordId(), invoiceId.toString());
                return null;
            }
        });
