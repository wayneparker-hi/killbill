...
public class DefaultInvoiceModule extends KillBillModule implements InvoiceModule {
    ...
    protected void installInvoicePluginApi() {
        bind(new TypeLiteral<PluginServiceRegistry<InvoicePluginApi>>() {}).toProvider(DefaultInvoiceProviderPluginRegistryProvider.class).asEagerSingleton();
    }

    protected void installInvoiceFormatterFactory() {
        bind(new TypeLiteral<PluginServiceRegistry<InvoiceFormatterFactory>>() {}).toProvider(DefaultInvoiceFormatterFactoryProviderPluginRegistryProvider.class).asEagerSingleton();
        ...
        Multibinder.newSetBinder(binder(), new TypeLiteral<PluginServiceRegistry<?>>() {})
                   .addBinding().to(new TypeLiteral<PluginServiceRegistry<InvoicePluginApi>>() {});
        Multibinder.newSetBinder(binder(), new TypeLiteral<PluginServiceRegistry<?>>() {})
                   .addBinding().to(new TypeLiteral<PluginServiceRegistry<InvoiceFormatterFactory>>() {});
    }
