...
public class DefaultEntitlementModule extends KillBillModule implements EntitlementModule {
    ...
    protected void installEntitlementPluginApi() {
        bind(new TypeLiteral<PluginServiceRegistry<EntitlementPluginApi>>() {}).toProvider(DefaultEntitlementProviderPluginRegistryProvider.class).asEagerSingleton();
        ...
        Multibinder.newSetBinder(binder(), new TypeLiteral<PluginServiceRegistry<?>>() {})
                   .addBinding().to(new TypeLiteral<PluginServiceRegistry<EntitlementPluginApi>>() {});
