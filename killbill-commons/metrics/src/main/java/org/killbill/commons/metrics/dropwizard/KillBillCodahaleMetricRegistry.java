...
public class KillBillCodahaleMetricRegistry implements MetricRegistry {
    ...
    /**
     * The Dropwizard registry this delegates to.
     * <p>
     * Exposed for reporters -- Graphite, InfluxDB, Prometheus -- which take a Dropwizard registry
     * and cannot be written against the Kill Bill interface alone. The alternative was for each of
     * them to re-implement the wrapping in reverse.
     *
     * @return the underlying registry; never null
     */
    @edu.umd.cs.findbugs.annotations.SuppressFBWarnings(
            value = "EI_EXPOSE_REP",
            justification = "A metric registry is shared mutable state by design: the point of "
                            + "handing it out is that reporters read the same metrics everything "
                            + "else writes. A copy would report a frozen snapshot.")
    public com.codahale.metrics.MetricRegistry getMetricRegistry() {
        return dwMetricRegistry;
    }
