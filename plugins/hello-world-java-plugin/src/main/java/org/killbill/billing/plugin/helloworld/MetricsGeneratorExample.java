...
public class MetricsGeneratorExample {
    ...
    public MetricsGeneratorExample(final MetricRegistry metricRegistry) {
        this.thread = new Thread(new Runnable() {
            public void run() {
                while (!stopMetrics) {
                    ...
                    try {
                        metricRegistry.counter("hello_counter").inc(1);
                    } catch (final PlatformServiceUnavailableException ignored) {
