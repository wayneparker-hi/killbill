...
public class HelloWorldListener implements NotificationPluginApi {
    ...
    /**
     * The formatter to use, or null when no plugin publishes one.
     * <p>
     * Was a {@code ServiceTracker} under OSGi, consulted on every use because the service could
     * come and go. The plugin runtime resolves it once at start, so a plain field is enough --
     * a plugin that needs to react to one appearing later can read the registry itself.
     */
    private final InvoiceFormatterFactory invoiceFormatterFactory;

    private final Properties configProperties;

    public HelloWorldListener(final KillbillApi killbillApi, final InvoiceFormatterFactory invoiceFormatterFactory, final Properties configProperties) {
        this.killbillApi = killbillApi;
        this.invoiceFormatterFactory = invoiceFormatterFactory;
    ...
    public void onEvent(final ExtBusEvent killbillEvent) {
        ...
        switch (killbillEvent.getEventType()) {
            ...
            case ACCOUNT_CHANGE:
                try {
                    final Account account = killbillApi.getAccountUserApi().getAccountById(killbillEvent.getAccountId(), context);
            ...
            case INVOICE_CREATION:
                ...
                try {
                    account = killbillApi.getAccountUserApi().getAccountById(killbillEvent.getAccountId(), context);
                } catch (AccountApiException e) {
                    throw new RuntimeException(e);
                }
                final List<Invoice> invoices = killbillApi.getInvoiceUserApi().getInvoicesByAccount(killbillEvent.getAccountId(), false, false, true, context);
                ...
                final InvoiceFormatterFactory formatterFactory = invoiceFormatterFactory;
                ...
                InvoiceFormatter invoiceFormatter = formatterFactory.createInvoiceFormatter(defaultLocale, null, invoice, Locale.forLanguageTag(account.getLocale()), killbillApi.getCurrencyConversionApi(), null, null);
