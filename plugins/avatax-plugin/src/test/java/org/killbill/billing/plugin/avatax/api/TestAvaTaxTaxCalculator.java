...
import org.killbill.billing.plugin.avatax.core.AvaTaxPlugin;
...
public class TestAvaTaxTaxCalculator extends AvaTaxRemoteTestBase {
    ...
    private KillbillApi killbillApi;
    ...
    public void setUp() throws Exception {
        ...
        killbillApi = TestUtils.buildKillbillApi(account);
        Mockito.when(killbillApi.getInvoiceUserApi()).thenReturn(Mockito.mock(InvoiceUserApi.class));
    }

    @Test(groups = "integration")
    public void testWithAvaTaxTaxCalculator() throws Exception {
        final AvaTaxConfigurationHandler avaTaxConfigurationHandler = new AvaTaxConfigurationHandler(AvaTaxPlugin.PLUGIN_NAME, killbillApi);
        avaTaxConfigurationHandler.setDefaultConfigurable(client);
        final AvaTaxTaxCalculatorBase calculator = new AvaTaxTaxCalculator(avaTaxConfigurationHandler, dao, clock, killbillApi);
    ...
    public void testExemptionWithAvaTaxTaxCalculator() throws Exception {
        final AvaTaxConfigurationHandler avaTaxConfigurationHandler = new AvaTaxConfigurationHandler(AvaTaxPlugin.PLUGIN_NAME, killbillApi);
        avaTaxConfigurationHandler.setDefaultConfigurable(client);
        final AvaTaxTaxCalculator calculator = new AvaTaxTaxCalculator(avaTaxConfigurationHandler, dao, clock, killbillApi);
    ...
    public void testWithTaxRatesTaxCalculator() throws Exception {
        final TaxRatesConfigurationHandler taxRatesConfigurationHandler = new TaxRatesConfigurationHandler(AvaTaxPlugin.PLUGIN_NAME, killbillApi);
        taxRatesConfigurationHandler.setDefaultConfigurable(taxRatesClient);
        final AvaTaxTaxCalculatorBase calculator = new TaxRatesTaxCalculator(taxRatesConfigurationHandler, dao, clock, killbillApi);
        testComputeItemsOverTime(calculator);
    }
    ...
    private void testComputeItemsOverTime(final AvaTaxTaxCalculatorBase calculator, final Account account) throws Exception {
        ...
        Mockito.when(killbillApi.getInvoiceUserApi().getInvoiceByInvoiceItem(Mockito.eq(taxableItem2.getId()), Mockito.<TenantContext>any()))
               .thenReturn(invoice);
