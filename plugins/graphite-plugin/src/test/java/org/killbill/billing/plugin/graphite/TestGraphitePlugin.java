...
/**
 * Reports Kill Bill's metrics to a Graphite server on a fixed interval.
 * <p>
 ...
 * reloaded plugin leaks metaspace and, worse, keeps writing to Graphite from a version that is no
 ...
 */
public class TestGraphitePlugin {

    private static final String PLUGIN_ID = "killbill-graphite";
    private static final String ENABLED = "org.killbill.metrics.graphite";
    ...
    public void testThePlatformPropertyEnablesIt() {
        // Port 2003 with nothing listening: Graphite connects lazily, on the first report, so the
        // reporter starts regardless -- which is what lets this run without a server.
    ...
    private static Set<String> reporterThreads() {
        return Thread.getAllStackTraces().keySet().stream()
                     .map(Thread::getName)
                     .filter(name -> name.toLowerCase(java.util.Locale.ROOT).contains("graphite"))
