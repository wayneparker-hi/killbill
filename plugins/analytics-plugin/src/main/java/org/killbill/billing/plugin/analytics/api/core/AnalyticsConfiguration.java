...
import org.killbill.billing.plugin.analytics.AnalyticsPlugin;
...
public class AnalyticsConfiguration {
    ...
    public AnalyticsConfiguration(final Properties properties) {  //CTOR used only for default configuration from global properties if available
        ...
        final String blackList = properties.getProperty(AnalyticsPlugin.PROPERTY_PREFIX + "blacklist");
        this.blacklist = blackList != null && !blackList.isEmpty() ? Arrays.asList(blackList.split(",")) : new LinkedList<>();

        final String ignoredGroups = properties.getProperty(AnalyticsPlugin.PROPERTY_PREFIX + "ignoredGroups");
        this.ignoredGroups = ignoredGroups != null && !ignoredGroups.isEmpty() ? Arrays.asList(ignoredGroups.split(",")) : new LinkedList<>();

        final String highCardinalityAccounts = properties.getProperty(AnalyticsPlugin.PROPERTY_PREFIX + "highCardinalityAccounts");
        ...
        this.refreshDelaySeconds = properties.getProperty(AnalyticsPlugin.PROPERTY_PREFIX + "refreshDelaySeconds") != null ? Integer.parseInt(properties.getProperty(AnalyticsPlugin.PROPERTY_PREFIX + "refreshDelaySeconds")) : 10;
        this.lockAttemptRetries = properties.getProperty(AnalyticsPlugin.PROPERTY_PREFIX + "lockAttemptRetries") != null ? Integer.parseInt(properties.getProperty(AnalyticsPlugin.PROPERTY_PREFIX + "lockAttemptRetries")) : 100;
        this.rescheduleIntervalOnLockSeconds = properties.getProperty(AnalyticsPlugin.PROPERTY_PREFIX + "rescheduleIntervalOnLockSeconds") != null ? Integer.parseInt(properties.getProperty(AnalyticsPlugin.PROPERTY_PREFIX + "rescheduleIntervalOnLockSeconds")) : 10;
        this.enablePartialRefreshes = properties.getProperty(AnalyticsPlugin.PROPERTY_PREFIX + "enablePartialRefreshes") == null || Boolean.parseBoolean(properties.getProperty(AnalyticsPlugin.PROPERTY_PREFIX + "enablePartialRefreshes"));
        this.enableTemplateVariables = properties.getProperty(AnalyticsPlugin.PROPERTY_PREFIX + "enableTemplateVariables") != null && Boolean.parseBoolean(properties.getProperty(AnalyticsPlugin.PROPERTY_PREFIX + "enableTemplateVariables"));
