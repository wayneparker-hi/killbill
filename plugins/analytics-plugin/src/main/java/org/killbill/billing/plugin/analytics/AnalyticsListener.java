...
import static org.killbill.billing.plugin.analytics.AnalyticsPlugin.ANALYTICS_QUEUE_SERVICE;

public class AnalyticsListener implements NotificationPluginApi {
    ...
    private final PluginConfigProperties osgiConfigPropertiesService;
    ...
    public AnalyticsListener(final KillbillApi killbillApi,
                             final DataSource osgiKillbillDataSource,
                             final MetricRegistry metricRegistry,
                             final PluginConfigProperties osgiConfigPropertiesService,
    ...
    public void onEvent(final ExtBusEvent killbillEvent) {
        ...
        if (group == Group.INVOICES) {
            try {
                ...
                final Invoice invoice = killbillApi.getInvoiceUserApi().getInvoice(killbillEvent.getObjectId(), tenantContext);
    ...
    public boolean scheduleAnalyticsJob(final AnalyticsJob job, final AnalyticsConfiguration analyticsConfiguration) {
        ...
        if (recordIdApi == null) {
            ...
            accountRecordId = killbillApi.getRecordIdApi().getRecordId(job.getAccountId(), ObjectType.ACCOUNT, callContext);
            tenantRecordId = killbillApi.getRecordIdApi().getRecordId(job.getTenantId(), ObjectType.TENANT, callContext);
        }
    ...
    private void handleAnalyticsJobWithLock(final AnalyticsJob job) throws AnalyticsRefreshException {
        ...
        final BusinessContextFactory businessContextFactory = new BusinessContextFactory(job.getAccountId(), callContext, currencyConversionDao, killbillApi, osgiConfigPropertiesService, clock, analyticsConfigurationHandler);
