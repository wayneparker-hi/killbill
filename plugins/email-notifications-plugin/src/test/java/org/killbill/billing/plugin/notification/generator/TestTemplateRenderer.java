...
public class TestTemplateRenderer extends TestBase {
    ...
    // testCreateInvoiceWithCustomFormatterFactory removed: it exercised the OSGi ServiceTracker
    // that supplied the formatter, which the plugin runtime replaced with a registry lookup.
    // A replacement belongs in a test that starts the plugin, not one that mocks a framework.
