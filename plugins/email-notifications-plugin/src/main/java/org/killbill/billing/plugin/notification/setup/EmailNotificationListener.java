...
public class EmailNotificationListener implements NotificationPluginApi {
    ...
    private final PluginConfigProperties configProperties;
    ...
    public EmailNotificationListener(final Clock clock, final KillbillApi killbillApi, final PluginConfigProperties configProperties,
                                     DataSource dataSource, EmailNotificationConfigurationHandler emailNotificationConfigurationHandler,
                                     final InvoiceFormatterFactory invoiceFormatterFactory) throws SQLException {
        ...
        this.templateRenderer.setInvoiceFormatterTracker(invoiceFormatterFactory);
        this.dao = new ConfigurationDao(dataSource);
    ...
    public void onEvent(final ExtBusEvent killbillEvent) {
        ...
        try {
            final Account account = killbillApi.getAccountUserApi().getAccountById(killbillEvent.getAccountId(), new EmailNotificationContext(killbillEvent.getAccountId(), killbillEvent.getTenantId()));
    ...
    private void sendEmailForUpComingInvoice(final Account account, final ExtBusEvent killbillEvent, final TenantContext context) throws IOException, InvoiceApiException, EmailException, TenantApiException, EmailNotificationException {
        ...
        final PluginCallContext callContext = new PluginCallContext(EmailNotificationPlugin.PLUGIN_NAME, now, context.getAccountId(), context.getTenantId());
        try {
        	final EmailNotificationConfiguration emailNotificationConfiguration = getConfiguration(context);
        	killbillApi.getSecurityApi().login(emailNotificationConfiguration.getAdminUsername(), emailNotificationConfiguration.getAdminPassword());
        	final Invoice invoice = killbillApi.getInvoiceUserApi().triggerDryRunInvoiceGeneration(
                    account.getId(),
    ...
    private void sendEmailForCancelledSubscription(final Account account, final ExtBusEvent killbillEvent, final TenantContext context) throws SubscriptionApiException, IOException, EmailException, TenantApiException, EmailNotificationException {
        ...
        final Subscription subscription = killbillApi.getSubscriptionApi().getSubscriptionForEntitlementId(subscriptionId, false, context);
    ...
    private void sendEmailForPayment(final Account account, final ExtBusEvent killbillEvent, final TenantContext context) throws InvoiceApiException, IOException, EmailException, PaymentApiException, TenantApiException, EmailNotificationException {
        ...
        final Invoice invoice = killbillApi.getInvoiceUserApi().getInvoice(invoiceId, context);
        ...
        final Payment payment = killbillApi.getPaymentApi().getPayment(invoicePayment.getPaymentId(), false, false, ImmutableList.<PluginProperty>of(), context);
    ...
    private void sendEmailForInvoiceCreation(final Account account, final ExtBusEvent killbillEvent, final TenantContext context) throws InvoiceApiException, IOException, TenantApiException, EmailException, EmailNotificationException {
        ...
        final Invoice invoice = killbillApi.getInvoiceUserApi().getInvoice(killbillEvent.getObjectId(), context);
        if (invoice != null) {
    ...
    private void sendEmail(final Account account, final EmailContent emailContent, final TenantContext context) throws IOException, EmailException, EmailNotificationException {
        final Iterable<String> cc = Iterables.transform(killbillApi.getAccountUserApi().getEmails(account.getId(), context), new Function<AccountEmail, String>() {
            @Override
