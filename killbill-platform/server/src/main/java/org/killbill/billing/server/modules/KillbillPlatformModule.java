...
import org.killbill.billing.platform.plugin.runtime.MapBackedPluginServiceRegistry;
import org.killbill.billing.platform.plugin.runtime.PluginRuntimeModule;
import org.killbill.billing.platform.plugin.runtime.SingleValuePluginServiceRegistry;
...
public class KillbillPlatformModule extends KillBillPlatformModuleBase {
    ...
    /**
     * Installs the plugin runtime.
     * <p>
     * Replaces {@code configureOSGI()}. Note what is gone besides the module itself: the OSGi layer
     * required its own database (a dedicated DataSource and EmbeddedDB, configured separately from
     * the main one) purely so that the Felix framework and its bundles had somewhere to keep state.
     * The plugin runtime keeps no state of its own, so there is nothing to configure.
     */
    protected void configurePluginRuntime() {
        install(new PluginRuntimeModule(configSource));
    }
    ...
    protected void configureMetrics() {
        final MapBackedPluginServiceRegistry<ServiceDiscoveryRegistry> serviceDiscoveryRegistry =
                new MapBackedPluginServiceRegistry<>(ServiceDiscoveryRegistry.class);
        bind(new TypeLiteral<PluginServiceRegistry<ServiceDiscoveryRegistry>>() {
        }).toInstance(serviceDiscoveryRegistry);
        Multibinder.newSetBinder(binder(), PluginRuntimeModule.REGISTRY_TYPE)
                   .addBinding().toInstance(serviceDiscoveryRegistry);

        final SingleValuePluginServiceRegistry<MetricRegistry> metricRegistryServiceRegistration =
                new SingleValuePluginServiceRegistry<>(MetricRegistry.class);
        bind(new TypeLiteral<SinglePluginServiceRegistry<MetricRegistry>>() {
        }).toInstance(metricRegistryServiceRegistration);
        Multibinder.newSetBinder(binder(), PluginRuntimeModule.SINGLE_REGISTRY_TYPE)
                   .addBinding().toInstance(metricRegistryServiceRegistration);
