...
public class KillbillPluginsMetricRegistry implements MetricRegistry {
    ...
    private final SinglePluginServiceRegistry<MetricRegistry> pluginMetricRegistry;

    @SuppressFBWarnings(value = "EI_EXPOSE_REP2",
                        justification = "The registry is an injected collaborator, deliberately shared. "
                                        + "Copying it would defeat its purpose: this class exists to forward "
                                        + "metrics to whichever registry a plugin registers, which means it "
                                        + "must observe that registry changing.")
    public KillbillPluginsMetricRegistry(final SinglePluginServiceRegistry<MetricRegistry> pluginMetricRegistry) {
